...
LOCALIZE_LOG_FILE = 'localize_log.txt'
...
def test_network_knife(model, network_file_path):
	...
	trial_differences = np.mean(all_differences, axis=1)


	# visualize the new actions compared to the old
	with PdfPages(re.sub('\..+', '.pdf', network_file_path)) as pdf:

		plot_actions(pdf, trial_differences, trial_differences, action_max, "Average Differences", zero_index=False)

		for image_idx, image_filename in enumerate(glob.glob('camera/*.png')):
...
def match_data(trials, avg_differences):
	...
	with PdfPages("avg_differences.pdf") as pdf:
		for trial_num, trial_differences in zip(trials, avg_differences):
			# remove first column
			trial_differences = np.delete(trial_differences, 0, axis=1)

			# plot diff sum
			trial_diff_sum = np.sum(trial_differences, axis=1)
			...
			output = pca.fit_transform(trial_differences)
			...
			both_differences.append(trial_differences[:, 0])
			both_differences.append(trial_differences[:, 1])
...
if __name__ == "__main__":
	try:
	    os.remove(LOCALIZE_LOG_FILE)
	except OSError:
	    pass
	actor_log = open(LOCALIZE_LOG_FILE, "a+")
